<?php include('header.php');?>
<div class="blcontainer">
	<div class="let_pl">
		<div class="img_pl" id="company">
			<div class="ab_pret" >
				<ul class="breadcram">
					<li>
						<a href="">Главная</a>
					</li>
					<li>
						/ О компании
					</li>
				</ul>
				<h1>О компании</h1>						
			</div>
		</div>
	</div>
</div>
<div class="blcontainer">
	<div class="second_block">
		<div class="block_o_company"> 
			<div class="row">
				<div class="col-md-4">
					<div class="photo_11" style="width: 315px;">
						<img src="img/noroot31.png" alt="" style="width: 315px; height: 262px;">
					</div>
				</div>
				<div class="col-md-8">
					<div class="block_text_o">
						<div class="galce_o">
							<img src="img/galck.svg" alt="">
						</div>
						<p>Наша компания начиналась с того, что мы оформляли ипотеку для себя. Пройдя весь этот 
							нелегкий путь, мы начали помогать с оформлением родственникам, друзьям и знакомым. 
						С тех пор прошло 14 лет, но подход к работе так и остался: "Как для себя".</p>
						<span class="at1">Лилия Решетникова</span>
						<span class="at2">Генеральный директор</span>
					</div>
					<div class="row block_2_tr">
						<div class="col-md-6">
							<div class="plum">
								>750 млн. ₽
								<p>Ипотечных кредитов оформили<br> нашим клиентам за 2019 год</p>
							</div>
						</div>
						<div class="col-md-6">
							<div class="plum">
								98,3%
								<p>Общий процент одобряемости <br>ипотечного кредита у наших клиентов</p>
							</div>
						</div>
						<div class="col-md-6">
							<div class="plum">
								14 лет
								<p>На рынке ипотечного кредитования<br>Москвы и других регионов РФ</p>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="blcontainer">
	<div class="second_block">
		<div class="row">
			<div class="col-md-12">
				<p class="title_trel">Наша команда</p>
			</div>
			<div class="col-md-9">
				<ul class="stet_sot">
					<li>
						<div class="img_bl">

						</div>
					</li>
					<li>
						<ul>
							<li>Александр Иванов</li>
							<li>Исполнительный директор</li>
							<li>
								<p>
									Наша компания начиналась с того, что мы оформляли 
									ипотеку для себя. Пройдя весь этот нелегкий путь, 
									мы начали помогать с оформлением родственникам, 
									друзьям и знакомым. С тех пор прошло 14 лет, но подход 
									к работе так и остался: "Как для себя".
								</p>
							</li>
						</ul>
					</li>
				</ul>
			</div>

			<div class="col-md-9 offset-md-3 mt-1-nb">
				<ul class="stet_sot">
					<li>
						<div class="img_bl">

						</div>
					</li>
					<li>
						<ul>
							<li>Александр Иванов</li>
							<li>Исполнительный директор</li>
							<li>
								<p>
									Наша компания начиналась с того, что мы оформляли 
									ипотеку для себя. Пройдя весь этот нелегкий путь, 
									мы начали помогать с оформлением родственникам, 
									друзьям и знакомым. С тех пор прошло 14 лет, но подход 
									к работе так и остался: "Как для себя".
								</p>
							</li>
						</ul>
					</li>
				</ul>
			</div>
			<div class="col-md-9 mt-1-nb">
				<ul class="stet_sot">
					<li>
						<div class="img_bl">

						</div>
					</li>
					<li>
						<ul>
							<li>Александр Иванов</li>
							<li>Исполнительный директор</li>
							<li>
								<p>
									Наша компания начиналась с того, что мы оформляли 
									ипотеку для себя. Пройдя весь этот нелегкий путь, 
									мы начали помогать с оформлением родственникам, 
									друзьям и знакомым. С тех пор прошло 14 лет, но подход 
									к работе так и остался: "Как для себя".
								</p>
							</li>
						</ul>
					</li>
				</ul>
			</div>
			<div class="col-md-9 offset-md-3 mt-1-nb">
				<ul class="stet_sot">
					<li>
						<div class="img_bl">
						</div>
					</li>
					<li>
						<ul>
							<li>Александр Иванов</li>
							<li>Исполнительный директор</li>
							<li>
								<p>
									Наша компания начиналась с того, что мы оформляли 
									ипотеку для себя. Пройдя весь этот нелегкий путь, 
									мы начали помогать с оформлением родственникам, 
									друзьям и знакомым. С тех пор прошло 14 лет, но подход 
									к работе так и остался: "Как для себя".
								</p>
							</li>
						</ul>
					</li>
				</ul>
			</div>

		</div>
	</div>
</div>
<div class="slider_s">
	<div class="left_block_s">
		<p>Офисная <br> жизнь</p>
	</div>
	<div class="right_block_s">
		<div class="slides">
			<ul>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>				
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
				<li>
					<img src="/img/Rectangle.png" alt="">		
				</li>
			</ul>
		</div>
		<span></span>
		<ul class="bt">
			<li class="lf"><a href=""></a></li>
			<li class="rt"><a href=""></a></li>
		</ul>
	</div>

</div>

<div class="blcontainer" style="margin-top: 130px;">
	<img src="/img/ewr.png" alt="">
</div>
<div class="blcontainer">
	<div class="second_block">
		<div class="row">

			<div class="col-md-4">
				<p class="title_trel">Что говорят <br>	наши клиенты</p>
			</div>
			<div class="col-md-8">
				<div class="dis_ordt">
					<div class="riwers_bl">
						<ul>
							<li>Валентина</li>
							<li>35 лет, Ярославль</li>
						</ul>
						<p>
							Больше всего боялась расчета с продавцом, деньги‑то огромные. Во‑первых, в банке не так страшно, 
							все‑таки охраняемый объект. Во‑вторых, хранение денег в банке и перевод их продавцу после 
							регистрации – такое меня вполне устроило. Отличный способ
						</p>
					</div>
					<div class="riwers_bl">
						<ul>
							<li>Валентина</li>
							<li>35 лет, Ярославль</li>
						</ul>
						<p>
							Больше всего боялась расчета с продавцом, деньги‑то огромные. Во‑первых, в банке не так страшно, 
							все‑таки охраняемый объект. Во‑вторых, хранение денег в банке и перевод их продавцу после 
							регистрации – такое меня вполне устроило. Отличный способ
						</p>
					</div>
					<div class="riwers_bl">
						<ul>
							<li>Валентина</li>
							<li>35 лет, Ярославль</li>
						</ul>
						<p>
							Больше всего боялась расчета с продавцом, деньги‑то огромные. Во‑первых, в банке не так страшно, 
							все‑таки охраняемый объект. Во‑вторых, хранение денег в банке и перевод их продавцу после 
							регистрации – такое меня вполне устроило. Отличный способ
						</p>
					</div>
				</div>
			</div>



			<div class="col-md-3">
				<p class="title_trel">Читайте новости <br> и статьи</p>
			</div>
			<div class="col-md-9">
				<div class="dis_ordt">
					<div class="block_fret_Y">
						<p>Как меньше платить<br> по действующей ипотеке</p>
						<p>Банки, государство, застройщики <br>
							регулярно улучшают условия <br>
							и возможности для клиентов, <br>
							собирающихся приобрести жилье <br>
							в ипотеку: снижают ставки, дают <br>
							скидки и льготы. Но как быть тем, <br>
						кто уже взял ипотеку?...</p>
						<a href="" class="more bel">Подробнее</a>
					</div>
					<div class="block_fret_Y">
						<p>Как меньше платить<br> по действующей ипотеке</p>
						<p>Банки, государство, застройщики <br>
							регулярно улучшают условия <br>
							и возможности для клиентов, <br>
							собирающихся приобрести жилье <br>
							в ипотеку: снижают ставки, дают <br>
							скидки и льготы. Но как быть тем, <br>
						кто уже взял ипотеку?...</p>
						<a href="" class="more cher">Подробнее</a>
					</div>
					<div class="block_fret_Y">
						<p>Как меньше платить<br> по действующей ипотеке</p>
						<p>Банки, государство, застройщики <br>
							регулярно улучшают условия <br>
							и возможности для клиентов, <br>
							собирающихся приобрести жилье <br>
							в ипотеку: снижают ставки, дают <br>
							скидки и льготы. Но как быть тем, <br>
						кто уже взял ипотеку?...</p>
						<a href="" class="more cher">Подробнее</a>
					</div>
					<div class="block_fret_Y">
						<p>Как меньше платить<br> по действующей ипотеке</p>
						<p>Банки, государство, застройщики <br>
							регулярно улучшают условия <br>
							и возможности для клиентов, <br>
							собирающихся приобрести жилье <br>
							в ипотеку: снижают ставки, дают <br>
							скидки и льготы. Но как быть тем, <br>
						кто уже взял ипотеку?...</p>
						<a href="" class="more bel">Подробнее</a>
					</div>							
				</div>

			</div>
			<div class="col-md-12 fl-l">
				<a href="" class="pur_link" style="margin-top: 25px;">Все новости</a>
			</div>
		</div>
	</div>
</div>
<?php include('footer.php')?>